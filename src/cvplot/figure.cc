...
namespace cvplot {
    ...
    /**
     * @data 1st el-x value in plot
     *       2nd el-y value in plot
     *       3rd el- status of point 
     *       4th el- label of the point
     */
    ...
    bool Series::legend() const {
        return legend_;
    }

    Color Series::color() const {
        return color_;
    }
        ...
        switch (type_) {
                ...
                for (const auto &e : entries_) {
                    ...
                    if (has_last) {
                        if (type_ == DotLine || type_ == Line || type_ == FillLine ||
                                type_ == RangeLine) {
                            cv::line(trans.with(color_),{(int) (last_x * xs + xd), (int) (last_y * ys + yd)}, point,
                                    color, 1, cv::LINE_AA);
                        }
                    } else {
                        has_last = true;
            ...
            case Histogram:
            {
                ...
                for (const auto &e : entries_) {
                    ...
                    if (type_ == Histogram) {
                        cv::rectangle(trans.with(color_),{(int) (x * xs + xd) - u + o, (int) (y_axis * ys + yd)},
                        {
                            (int) (x * xs + xd) + u + o, (int) (y * ys + yd)
                        }, color,
                                -1, cv::LINE_AA);
                    } else if (type_ == Vistogram) {
                        cv::rectangle(trans.with(color_),{(int) (x_axis * xs + xd), (int) (x * ys + yd) - u + o},
                        {
                            (int) (y * xs + xd), (int) (x * ys + yd) + u + o
                        }, color,
                                -1, cv::LINE_AA);
                    }
                }
    ...
    Color Figure::backgroundColor() {
        return background_color_;
    }

    Color Figure::axisColor() {
        return axis_color_;
    }

    Color Figure::subaxisColor() {
        return sub_axis_color_;
    }
    ...
    void Figure::draw(void *b, float x_min, float x_max, float y_min, float y_max,
        ...
        for (const auto &s : series_) {
            ...
            auto shadow = true;
            cv::putText(trans.with(background_color_), name.c_str(),{org.x + (shadow ? 1 : 0), org.y + (shadow ? 1 : 0)},
            cv::FONT_HERSHEY_SIMPLEX, 0.4f, color2scalar(background_color_),
                    (shadow ? 1.f : 2.f));
            cv::circle(trans.with(background_color_),{buffer.cols - border_size_ - 10 + 1, org.y - 3 + 1}, 3,
